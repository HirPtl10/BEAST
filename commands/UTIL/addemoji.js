...
let emoji = args[0]
if (!link) {
    if (!emoji && !message.attachments.first() && !link) return message.channel.send("**<:aloi_aviso:777754787855859714> | Você precisa especificar um (link) um (emoji) uma (imagem) ou (random)**");
...
.setTitle("<:aloi_sucesso:777618244525490216> | Sucesso")
.setColor("#8b6eff")
...
if(treco > 256000) return message.channel.send(`**<:aloi_aviso:777754787855859714> | Esta (imagem) pesa \`${peso}\` e necessário ser menor que \`256KB\`**`)
...
if(!nome) return message.channel.send("Give args")

} else {
return message.channel.send("*Give link or emoji");
}
} else if (link && !message.attachments.first() && isEmoji(args[0]).response === false) {
link = args[0]
nome = args.slice(1).join("")
if(!nome) return message.channel.send("Bruh")

const a = new Discord.MessageAttachment(link,  "foto.gif")
message.channel.send(a).then((s) => {
if(s.attachments.first().size >= 256000) {
message.channel.send("More than \`256KB\`")
}
size = s.attachments.first().size
s.delete()
})

if(!link.endsWith('.gif') && !link.endsWith('.png') && !link.endsWith('.jpg') && !link.endsWith('.webp') && !link.endsWith('.jpeg')) return message.channel.send("**<:aloi_aviso:777754787855859714> | A sua imagem/link deve terminar com**\n**[**\`.gif\`**|**\`.png\`**|**\`.jpg\`**|**\`.webp\`**]**")
}
